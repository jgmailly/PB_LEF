...
# add variables
agent_vars = [model.NewIntVar(0, n_agents - 1, f"agent_{i}") for i in range(n_agents)]

# add Constraints
model.AddAllDifferent(agent_vars)
def lamb(ordObj):
    def inner(i,j):
...
for i in range(len(agents)):
    ordObj = preferences[agents.index(agents[i])]
    ordObj = [obj for obj in ordObj if len(obj) > 0] #clean the preference (it has an empty "" usually)
    ordObj_num = [objects.index(obj) for obj in ordObj]
    for j in range(len(agents)):
        if ([agents[i],agents[j]] in social) or ([agents[j],agents[i]] in social):
            model.AddAllowedAssignments([agent_vars[i],agent_vars[j]], [(k,m) for m in range(n_agents) for k in ordObj_num[:ordObj_num.index(m)] ])
