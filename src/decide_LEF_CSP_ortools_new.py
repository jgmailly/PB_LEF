...
# add variables
# assignment vars
assign_vars = [[model.NewBoolVar(f"assign_({i},{j})") for j in range(n_agents)] for i in range(n_agents)]

# add Constraints
for i in range(len(assign_vars)):
    model.Add(sum(assign_vars[i]) == 1)
    model.Add(sum(assign_vars[j][i] for j in range(n_agents)) == 1)
    ...
    for agent_j in agents:
        if ([agent_i,agent_j] in social) or ([agent_j,agent_i] in social):
            for i in range(len(ordObj)):
                ...
                alloc = assign_vars[agents.index(agent_j)][objects.index(ordObj[i])].Not()
                for obj in ordObj[:i]:
                    alloc += assign_vars[agents.index(agent_i)][objects.index(obj)]
                model.Add(alloc >= 1)
